...
| Worker suspension and restart tests | **Partial** — synthetic Chromium E2E terminates the service worker through CDP and verifies the persisted AgentSession/workspace recover; interruption during a provider stream and a mid-effect restart remain unverified (`npm run test:agent`) |
