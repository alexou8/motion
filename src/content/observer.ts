/**
 * Motion's content-script observer.
 *
 * It runs inside a page Motion does not control. It reads, and it reports.
 * Extraction is a pure `(url, document) -> domain` call. The actor
 * (`actor.ts`) is the only part of the content script that ever changes a
...
export function extract(requestId: string): void {
  const url = currentUrl();
  const adapter = resolveAdapter(url);
  if (!adapter) return;
 ...
 */
export function readContent(): { content: unknown } | { refused: string } {
  const url = currentUrl();
  const adapter = resolveAdapter(url);
